...
def get_dict(filename, n):
    ...
    ngram_dict = dict.fromkeys(unique_ngrams, 0)
...
def top_N_common(filename, N, n, threshold=0):
    ...
    # 2. sort the dict
    # fill in
    filtered_items = [(k, v) for k, v in ngram_dict.items() if v >= threshold]
    sorted_items = sorted(filtered_items, key=lambda x: (-x[1],-ord(x[0][0]) if x[0] else 0, x[0]), reverse=False)
    sorted_items = sorted(filtered_items, key=lambda x: (-x[1], x[0]), reverse=False)
    ...
    top_n_list = sorted_items[:N]
    top_n_grams = dict(top_n_list)
...
def compare_langs(test_file, langFiles, N, n=3):
    ...
    # fill in
    mystery_dict = top_N_common(test_file, N, n, threshold=0)
    ...
    for lang_file in langFiles:
        lang_dict = top_N_common(lang_file, N, n, threshold=0)
        lang_set = set(lang_dict.keys())
        inter = mystery_set.intersection(lang_set)
